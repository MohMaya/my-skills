...
3. **Recommend one library**, state what it's for in one sentence, and install/wire it up if that's part of the request. Don't present a menu of options when the list has a clear answer. A library not already installed is a new dependency: run `security-and-hardening` on it and get Shiv's go-ahead before installing.
