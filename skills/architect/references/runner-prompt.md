...
- Validate at boundaries, trust types inside, per `STANDARDS/Core/Code.md`. Business logic as pure functions. The shell stays thin.
...
You are one of several independent runners, each in a fresh context. Produce the best design you can make. Don't hedge against the others. Differences between candidates are the signal used to pick a base and graft. Converging on a safe-looking middle defeats the exploration.
