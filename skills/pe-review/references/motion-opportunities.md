...
1. **Never modify source code.** This skill reports; it does not implement. If asked to build a suggestion, hand it off (e.g. a motion audit `plan <description>` per `motion-audit.md`, pe-build motion mode to implement it, or let the user take the recipe to any agent).
...
One short paragraph: how much motion this interface actually needs, whether it's already close to right, and which single suggestion has the highest leverage. Close by pointing at the handoff: a motion audit `plan <suggestion>` (`motion-audit.md`) to turn any row into a self-contained implementation plan.
