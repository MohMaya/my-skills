...
0. **Locate the target.** Find the verification skill to maintain: the project-local skill whose body has launch/drive sections and a feature map (usually `verify-*/` in the skill folder the repo already keeps, such as `.claude/skills/` or `.agents/skills/`). Several candidates → ask which one; none → stop and point at `/create-verification-skill` instead of inventing a target.
...
6. **Ship or stop.** For changed: one PR of proven corrections, re-read every changed file first. Open it when Shiv asked for delivery, otherwise commit on a working branch and offer the PR. Ship it per `~/.agents/STANDARDS/Workflow/Git.md` and `create-pr`. For clean or blocked: no PR, report the outcome and the coverage honestly.
