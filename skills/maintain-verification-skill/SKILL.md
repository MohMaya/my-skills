...
6. **Ship or stop.** For changed: one PR of proven corrections, re-read every changed file first. Ship it per `~/.agents/STANDARDS/Workflow/Git.md` and `create-pr`. For clean or blocked: no PR, report the outcome and the coverage honestly.
